---
title: "explaintrust"
subtitle: "Test post-hoc explanations before acting on them"
author: "lvcheer"
date: last-modified
description: >
  A reproducible Python toolkit and interactive guide for evaluating the
  faithfulness, stability, and reproducibility of tabular SHAP and LIME
  explanations.
bibliography: references.bib
---

> **Version note (2026-09-21):** The figures and numerical examples below were
> regenerated together under the full-background SHAP/LIME protocol. Method
> disagreement and subgroup heterogeneity remain descriptive diagnostics and are
> excluded from the library's overall quality-check verdict.

::: {.project-hero}
SHAP and LIME can produce a polished explanation without showing whether it
tracks the model, survives another run, or changes across observed groups.
**explaintrust** turns those questions into explicit, reproducible checks for
numeric tabular classification and regression workflows.

::: {.project-actions}
[Open the live demo](https://lvcheer-explaintrust-appstreamlit-app-x7k48h.streamlit.app/){.btn .btn-primary}
[View the code](https://github.com/lvcheer/explaintrust){.btn .btn-outline-primary}
[Read the technical brief](https://github.com/lvcheer/explaintrust/blob/main/output/pdf/explaintrust-technical-brief.pdf){.btn .btn-outline-primary}
:::
:::

## Project at a glance

### Problem

An attribution plot describes an explainer's output. It does not establish that
the explanation is faithful to the model, stable under small perturbations,
reproducible across stochastic runs, or consistent across observed subgroups.
It also does not support a causal claim. explaintrust records evidence for these
questions without collapsing all of them into one universal reliability score.

### Evaluation map

| Question | Metric family | Report role |
|---|---|---|
| Do important features move the prediction? | SHAP removal-effect correlation and comprehensiveness; normalized LIME infidelity | scored |
| Does the explanation survive perturbation and reruns? | max-sensitivity; top-k rank and sign stability | scored |
| Do SHAP and LIME tell different stories? | sign, rank, top-k, and magnitude differences | descriptive |
| Does the story differ across selected groups? | cross-segment rank stability and top-k flip rate | descriptive |

The scored checks use versioned decision-policy defaults. Method and subgroup
diagnostics remain descriptive because disagreement or heterogeneity does not,
by itself, identify a bad explanation.

### Protocol and evidence

The repository contains a controlled synthetic calibration and a real-data
benchmark on Adult and Diabetes. Both use frozen configuration, seeded runs,
training-only preprocessing, explicit SHAP/LIME backgrounds, raw and summary
exports, environment provenance, and generated public claims.

- Exactly three synthetic metrics met the held-out separation rule: SHAP
  removal-effect correlation, LIME infidelity, and max-sensitivity.
- Across 48 Adult/Diabetes run aggregates, the pooled top-k stability median was
  `0.875`. Restricting the refreshed runs to the first explained instance
  restores `1.000`, so broader coverage exposed previously hidden variability.
- Comprehensiveness is heavy-tailed: its pooled median is `20.67`, while P90 is
  `8.112e+09`. Near-zero random-removal denominators make this a not-noise gate,
  not a graded effect size.

### Limits

The evidence covers two tabular datasets, SHAP, LIME, binary classification,
and regression. Several protocol corrections changed together, while the
historical benchmark has no raw runs; the refresh therefore cannot isolate
individual causes or prove improved explanation quality. The thresholds are
documented defaults rather than universal calibration, and the project has not
been peer reviewed.

### Run it

```bash
git clone https://github.com/lvcheer/explaintrust.git
cd explaintrust
python3 -m venv .venv
source .venv/bin/activate
python -m pip install -e .
python examples/demo.py
```

For citation metadata, use [`CITATION.cff`](https://github.com/lvcheer/explaintrust/blob/main/CITATION.cff).

## Interactive guide: why your SHAP plot might be lying to you {#interactive-guide}

::: {.lead}
SHAP and LIME produce clear visual summaries. This guide examines what evidence
would support acting on them, and where that evidence stops.
:::

## The picture that feels like understanding

There is a moment every data scientist recognizes: you run
`shap.summary_plot(model, X)`, and out comes a clean, ranked list of features
with red and blue dots. The plot has axes, a color scale, and the quiet
authority of a peer-reviewed figure. "Now I understand my model," you think. The
internals that were opaque a minute ago suddenly have a story with heroes and
villains — *this* feature pushes the prediction up, *that* one drags it down.

The feeling is real, and it is almost never earned. A bar chart of attribution
values is a **description of a computation**, not evidence that the description
is correct. The plot can be beautiful, internally consistent, and faithful to
absolutely nothing. That gap — between "the explanation looks right" and "the
explanation *is* right" — is the whole subject of this article.

> **The question this article asks is not "how do I explain my model?", but
> "what evidence supports acting on this explanation?"**

The reason this gap goes unchecked is structural. The tools we reach for — SHAP,
LIME, and the dozens of wrappers around them — are built to *produce*
explanations, and producing one is the end of their job. A library that hands
you a beautiful attribution plot has no incentive to ask whether that plot
tracks the model, survives a rerun, or holds up on a different slice of the
population. Checking is someone else's problem — which in practice means
nobody's. The tools that *check* explanations are rarer and far less polished,
because "can I trust this explanation?" is a much less marketable question than
"look, your model is now explained!"

## Four ways an explanation can lie

An explanation can look great and still be wrong in (at least) four independent
ways. You should hold every explanation up against all four.

### 1. It isn't faithful

Faithfulness is the first and most important property, and the one a pretty plot
hides most completely. An explanation is *faithful* when it tracks what the
model actually does: perturb the input, and the change the explanation predicts
should match the change the model produces [@yeh2019infidelity]. If the
explanation says feature `x2` is responsible for this prediction, then changing
`x2` — and nothing else — should move the prediction by an amount proportional
to its attribution. A SHAP plot never tells you whether this is true; it just
shows you numbers, and numbers always look confident.

Two cheap checks expose the gap:

- **Removal-effect correlation** — if a feature is ranked important, removing it
  should actually move the prediction.
- **Comprehensiveness** — removing the top-k features should move the prediction
  more than removing k random ones.

On the collinear dataset used throughout this article, the model's SHAP
explanations score a **removal-effect correlation of ≈ <!-- BEGIN AUTO:faithfulness.removal_effect_correlation -->0.69<!-- END AUTO:faithfulness.removal_effect_correlation -->** and a
**comprehensiveness ratio of ≈ <!-- BEGIN AUTO:faithfulness.comprehensiveness_ratio -->3.20<!-- END AUTO:faithfulness.comprehensiveness_ratio -->** — removing the top-3 features moves the
prediction roughly three times more than removing 3 random features. This ratio
is a tabular adaptation inspired by comprehensiveness-style ablation
[@deyoung2020eraser], not the original ERASER metric verbatim.
These averages are encouraging, but they are not a certificate. They are also
*earned*, not assumed: the same model
could just as easily have produced a gorgeous summary plot with a removal
correlation near zero, and the plot would look identical. "Good" here is not a
property of how the chart looks, but of how the explanation behaves under
ablation — and you have to measure that to know it.

### 2. The explainers disagree

SHAP and LIME are the two most widely used post-hoc explainers, and they do not
answer the same question. SHAP, in its modern form, is built on game-theoretic
Shapley values [@lundberg2017shap]; LIME fits a local linear surrogate around the
instance being explained [@ribeiro2016lime]. Both are reasonable, both are widely
trusted — and they frequently disagree, sometimes about the *direction* of a
feature's effect, sometimes only about its *rank*. When two reasonable methods
tell different stories about the *same prediction*, neither story deserves your
automatic trust [@krishna2024disagreement]. Disagreement is not proof that either
method is broken; it is proof that the attribution is *under-determined*, and
that the honest answer is "it depends" rather than a confident bar chart.

The punchline — and the part almost everyone gets wrong — is that **a big chunk
of the "disagreement" you read about is a category error**. Try the toggle
below: compare SHAP against LIME the *wrong* way, then the *right* way.

```{ojs}
// The centerpiece. data comes from article/scripts/generate_figures.py
data = FileAttachment("figures/conversion.json").json()
viewof use_contrib = Inputs.toggle({label: "Put LIME on the same scale as SHAP (contribution scale)", value: true})
lime = use_contrib ? data.contrib_lime : data.raw_lime
rank_corr = use_contrib ? data.after_rank_corr : data.before_rank_corr
```

```{ojs}
// Big number readout
display(htl.html`<div class="big-number">SHAP vs LIME rank correlation:
  <span class="${rank_corr > 0.4 ? 'good' : 'bad'}">${rank_corr.toFixed(2)}</span></div>
  <div style="color:#666">averaged over ${data.n_instances} instances · +1 = perfect agreement, 0 = none, −1 = opposite</div>`)
```

```{ojs}
// SHAP vs LIME scatter. Agreement = points on the diagonal.
scatter_data = data.features.map((f, i) => ({feature: f, shap: data.shap[i], lime: lime[i]}))
Plot.plot({
  height: 320,
  marks: [
    Plot.line([[-0.7, -0.7], [0.7, 0.7]], {stroke: "#ccc", strokeDasharray: "4 4"}),
    Plot.dot(scatter_data, {x: "shap", y: "lime", r: 6, fill: "#636EFA", tip: true})
  ],
  x: {label: "SHAP attribution"},
  y: {label: use_contrib ? "LIME attribution (contribution scale)" : "LIME attribution (raw weights)"},
  style: {fontSize: 12}
})
```

Two independent mistakes can distort measured disagreement. The first is
**output space**. A classifier's SHAP output depends on the explainer and model:
for example, TreeSHAP may explain probability for a random forest but raw margin
for gradient boosting. LIME may fit probability or another chosen model output.
Comparing attributions from different outputs is like comparing Celsius to
Fahrenheit without converting: the numerical agreement is not interpretable.

The second mistake is subtler and matters more. SHAP values are
**contributions**: they satisfy `Σ φ_i ≈ f(x) − E[f(x)]`, a statement about how
far this prediction sits from the baseline. LIME weights are **slopes**: they
satisfy `f(x̃) ≈ f(x) + φ·(x̃ − x)`, a statement about the local rate of change.
A slope and a contribution are different quantities with different units, and
feeding one into a metric designed for the other is a category error
[@yeh2019infidelity]. Matching the model output and rescaling LIME's slopes onto
an approximate contribution scale is what the toggle above illustrates. In
this dataset the rank correlation changes from
**<!-- BEGIN AUTO:agreement.before_rank_corr -->0.68<!-- END AUTO:agreement.before_rank_corr --> to <!-- BEGIN AUTO:agreement.after_rank_corr -->0.75<!-- END AUTO:agreement.after_rank_corr -->**, while the
sign-disagreement rate drops from
<!-- BEGIN AUTO:agreement.before_sign_disagreement -->0.54<!-- END AUTO:agreement.before_sign_disagreement --> to <!-- BEGIN AUTO:agreement.after_sign_disagreement -->0.26<!-- END AUTO:agreement.after_sign_disagreement -->. The rank and sign changes show why
unit alignment must precede interpretation of residual disagreement.

::: {.quote}
Some apparent disagreement is a measurement artifact. Align the estimands and
units first; investigate the disagreement that remains.
:::

### 3. It doesn't reproduce

Take the same instance and the same trained model, and explain it twice with a
different random seed. If the two explanations differ, the explanation is not a
fact about your model — it is partly an artifact of the explainer's internal
coin flips. Stochastic methods such as KernelSHAP and LIME sample perturbations
to approximate their answer, so they *will* vary from run to run; the only
question is how much. We measure this with run-to-run **rank correlation**,
**sign agreement**, and **top-k overlap**. On the current example, LIME's
all-feature rank correlation is ≈ <!-- BEGIN AUTO:stability.rank_corr -->0.73<!-- END AUTO:stability.rank_corr -->, its top-3 rank correlation is <!-- BEGIN AUTO:stability.topk_rank_corr -->1.00<!-- END AUTO:stability.topk_rank_corr -->, and
its sign agreement is <!-- BEGIN AUTO:stability.sign_agreement -->1.00<!-- END AUTO:stability.sign_agreement -->. Those views answer different questions; none is
enough on its own to justify action.

TreeSHAP, by contrast, is deterministic and therefore trivially stable. Watch
the trap here: *determinism is not correctness*. A wrong answer that never
changes is stable, not trustworthy. Stability is necessary; it is never
sufficient.

### 4. It isn't consistent across subgroups

A single explanation is a point estimate, and a point estimate can be locally
true and globally false. The question the previous three checks do not answer is
whether the *story* — the ranking of important features — holds across observed
subpopulations. Split your evaluation data into prespecified segments (by a
demographic, a cluster, or time), recompute global feature importance within
each, and ask whether the ranking stays stable. We report **cross-segment rank stability**
(the correlation of importance rankings between segments) and the **top-k flip
rate** (how often a segment's top features differ from the reference). A change
between segments may faithfully reflect different model behavior across groups;
it is not by itself evidence of explanation failure. This is a heterogeneity check, not a direct test of
source-to-target distribution shift; drift evaluation needs separate source and
deployment samples.

## A worked example: collinearity quietly lies to you

To make these failures concrete, here is a deliberately small experiment with a
known data-generating process. We generate data where `x0` and `x1` directly
enter the label mechanism, while `x2` is a noisy proxy for `x0` but does not
enter that mechanism. The remaining features are pure noise. A predictive model
can nevertheless use `x2`, so a faithful model explanation may correctly assign
it credit without establishing that `x2` has a causal effect.

![Collinearity shifts model-level credit toward the non-causal proxy x2.](figures/endpoints.png)

The two panels show the same analysis at two levels of collinearity. On the left
(empirical correlation `r = <!-- BEGIN AUTO:endpoints.profiles.clean.empirical_x0_x2_correlation -->0.05<!-- END AUTO:endpoints.profiles.clean.empirical_x0_x2_correlation -->`), both methods assign little model-level importance to
`x2`. On the right (`r = <!-- BEGIN AUTO:endpoints.profiles.collinear.empirical_x0_x2_correlation -->0.86<!-- END AUTO:endpoints.profiles.collinear.empirical_x0_x2_correlation -->`), both explainers assign the proxy more model-level credit.
This is not proof that either method has recovered causality. It shows that the
model's predictive reliance can be redistributed among correlated variables.

Why does this happen? When `x2` is nearly indistinguishable from `x0`, the
*prediction* barely changes whether the model uses one or the other — so the
attribution is **under-determined**. There is no single correct answer to "how
much did `x0` contribute?", because the data cannot separate `x0`'s contribution
from `x2`'s. Any reasonable method must break the tie arbitrarily, and different
methods break it differently. Notice what this means for the trust battery: the
model may predict well while its attribution remains ambiguous. The error occurs
when a model-behaviour explanation is presented as a causal story; neither SHAP
nor LIME supports that interpretation by itself.

## So what should you do instead?

A trust battery — run these before you trust any post-hoc explanation:

| Check | Metric | Good sign |
|---|---|---|
| Faithful? | removal-effect correlation, comprehensiveness | corr high; ratio ≫ 1 |
| Reproducible? | run-to-run rank/sign/top-k stability | corr ≈ 1 |
| How do methods differ? | SHAP vs LIME agreement (same scale) | differences interpreted in context |
| How do observed groups differ? | cross-segment stability, top-k flip rate | heterogeneity examined in context |

The checklist has one principle behind it: **measure, don't eyeball.** Stop
asking whether an explanation *looks* right and start asking whether it survives
a set of simple, reproducible perturbations. Just as important, keep the two
stages separate: the metrics *measure* (a correlation, a flip rate), and only
you can *interpret* (does a rank correlation of 0.6 matter for your use case,
with your model, for your audience?). The thresholds in any library — including
the one below — are sensible starting points, not laws of nature. Calibrate them
against what you care about, and treat a passing score as permission to look
closer, never as a certificate.

The library that produced every number in this article is open source:
[**explaintrust**](https://github.com/lvcheer/explaintrust) — a small,
opinionated Python package that runs this battery for you.

## Caveats

A few honest caveats. explaintrust is at v0.1 and currently targets **numeric
tabular binary-classification and regression workflows with SHAP and LIME** —
multiclass evaluation, image, text, LLM interpretability, and counterfactual
explanations are out of scope for now. Its subgroup checks do not measure
deployment drift. The report thresholds are *documented defaults, not calibrated
claims*; they should be overridden using domain evidence. And the deepest caveat
of all: these metrics
are **diagnostics, not universal requirements for correctness**. No single number — and no battery of
numbers — certifies an explanation. Explainability is not a badge you print; it
is a property of the model, the data, and the audience together. The best these
checks can do is turn "I trust this because it looks nice" into "I trust this
because I tested it" — and that, it turns out, is most of the way there.

## References {.unnumbered}

::: {#refs}
:::
