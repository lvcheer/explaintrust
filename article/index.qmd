...
> **Version note (2026-09-05):** The figures and numerical examples below are
> historical snapshots. The library now aligns the full SHAP/LIME background;
> these examples have not yet been regenerated to evaluate that updated protocol.
> Method disagreement and subgroup heterogeneity are now descriptive diagnostics,
...
**comprehensiveness ratio of ≈ 5.1** — removing the top-3 features moves the
prediction roughly five times more than removing 3 random features. This ratio
...
this dataset the rank correlation changes from **0.74 to 0.75**, while the
sign-disagreement rate drops from 0.54 to 0.29. The small rank change is itself
informative: unit correction matters here mainly for direction, not ordering.
...
all-feature rank correlation is ≈ 0.72, its top-3 rank correlation is 1.0, and
its sign agreement is ≈ 0.67. Those views answer different questions; none is
...
(correlation `ρ ≈ 0`), both methods assign little model-level importance to
`x2`. On the right (`ρ ≈ 0.85`), LIME assigns the proxy more credit, in some
cases nearly as much as `x0`; SHAP is less affected in this particular setup.
